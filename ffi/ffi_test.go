...
func TestCallPrintf(t *testing.T) {
	...
	sym, err := GetSymbol(handle, unsafe.Pointer(&funcName))
	if err != nil {
	...
	str := "Hello, WebGPU!\n\x00" // Строка с завершающим нулём
	// ПЕРЕДАЕМ НЕПОСРЕДСТВЕННО УКАЗАТЕЛЬ НА ДАННЫЕ!
	...
	if runtime.GOOS == "windows" {
		...
		// puts returns non-negative number on success
		if retVal < 0 {
